...
def havaIter(bb, BBGT):
    ...
    inters = np.max(iw * ih)
    return inters>=1.
def myGen(pal,par,neg, cind=0):
    # Left: 1 Right: 1 neg: 1
    return ['C1', 'C1', 'C3'][cind%3], [pal, par, neg][cind%3][cind/3]
if __name__ == '__main__':
    ...
    """
    jsonPath = '/home/wanghao/Desktop/newVOC/label_001-660.json' # '/home/wanghao/AL_5600-5710.json'
    imagePath = '/home/wanghao/Desktop/newVOC/VOC2007/JPEGImages' # '/home/wanghao/TFS/AL_1101-6000'
    negPath = '/home/wanghao/Desktop/newVOC/crops' # 负样本子图扣件
    palPath = '/home/wanghao/Desktop/crops/roi-LM'  # 负样本子图扣件
    parPath = '/home/wanghao/Desktop/crops/roi-RM'  # 负样本子图扣件
    # save new image dir
    createDirs(pDir)
    annos = parseFromFile(jsonPath)
    neglist = glob.glob(negPath+'/*.*')
    pallist = glob.glob(palPath + '/*.*')
    parlist = glob.glob(parPath + '/*.*')
    cind = 0 # cropImage 索引
    ...
    for item in annos:
        ...
        for i in range(6):
            cls, cropName = myGen(pal=pallist, par=parlist, neg=neglist,cind=cind)
            cropImage = cv.imread(cropName) # croplist[cind]
            cind += 1
